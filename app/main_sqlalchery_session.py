...
def select1():
    ...
    datalist = list()
    result = dbsession.execute(text("SELECT * FROM employee_master")).mappings()
    for row in result:
...
def insert1():
    ...
    querydict = {"emp_hame": "Atul Test", "email": "test@yopmail.com", "mobile": "123000000","status":"A","password":"$2y$10$T.6Dx9YvqhSQzEioDCDoC.MG5TY/TYmA3gHksXO2xZIvAxBGjPV7q","Login_Route":"web"}
    dbsession.execute(text(query),querydict)
    dbsession.commit()
    return {"employee":datalist}
...
def update1():
    ...
    querydict = {"mobile": "1234560000","id":3}
    dbsession.execute(text(query),querydict)
    dbsession.commit()
    return {"employee":datalist}
